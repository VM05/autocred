<template>
  <div class="relative py-6 w-full">
    <label :for="props.id" v-if="props.label" class="text-primary-900 font-bold text-base">{{
      props.label
    }}</label>
    <input
      required
      ...
      @input="format"
    />
    <span class="text-red-700 block text-center absolute left-1/2 -translate-x-1/2 w-full">{{
      validated
    }}</span>
  </div>
</template>
