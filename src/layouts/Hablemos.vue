<template>
  <div class="py-6 md:py-12 px-6 md:px-12 md:mb-12 mb-6">
    ...
    <div class="bg-transparent md:bg-hablemos-100 rounded-xl card p-0 md:px-6 md:py-7 md:shadow-xl">
      <Heading1 content="Formulario Contacto" headingType="h3" class="border-b pb-4" />
      <div class="grid place-content-center my-20" v-if="alerts">
        ...
        <div v-if="isSuccess" class="text-center">
          ...
          <Paragraph class="mb-8"
            >Estamos buscando a tu copiloto y este se comunicará contigo a la brevedad.
          </Paragraph>
        ...
        <div v-if="isError" class="text-center">
          ...
          >
          <Paragraph class="mb-8 text-secondary-900">Intenta de nuevo </Paragraph>
      ...
      >
        <input type="hidden" name="user_servicios" :value="formContacto.servicios" />
        ...
        <div class="right md:pl-8 p-0">
          ...
          <div class="flex justify-end">
            <ButtonVue text="Enviar" outlinePrimary type="submit" :disabled="warnings.isWarning" />
          </div>
        </div>
      </form>
    </div>
...
<script setup>
  ...
  const enviarMail = () => {
    const formulario = document.getElementById('form');
    // emailjs.sendForm('service_pgv1taa', 'template_seiqd1b', formulario, 'Jm7OFS9JSVMS2XjET');
  };
</script>
